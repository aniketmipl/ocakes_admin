...
class Datatables {
    ...
    public function or_where_in($key_condition, $val = NULL, $backtick_protect = TRUE) {
        ...
        $this->ci->db->or_where_in($key_condition, $val, $backtick_protect);
        return $this;
    }
// $this->db->or_where_in();
    ...
    private function get_paging() {
        ...
        $iStart = intval($this->ci->input->post('iDisplayStart', TRUE));
        $iLength = intval($this->ci->input->post('iDisplayLength', TRUE));
    ...
    private function get_ordering() {
        $sColumns = explode(",", htmlspecialchars($this->ci->input->post('sColumns', TRUE)));

        if ($this->ci->input->post('iSortCol_0', TRUE) || $this->ci->input->post('iSortCol_0', TRUE) == 0) {
            for ($i = 0; $i < intval($this->ci->input->post('iSortingCols', TRUE)); $i++) {
                if ($this->ci->input->post('bSortable_' . intval($this->ci->input->post('iSortCol_' . $i, TRUE)), TRUE) == "true") {
                    $this->ci->db->order_by($sColumns[$this->ci->input->post('iSortCol_' . $i,true)], $this->ci->input->post('sSortDir_' . $i,true));
    ...
    private function get_filtering() {
        $sColumns = explode(",", $this->ci->input->post('sColumns', TRUE));

        $like = array('local' => array(), 'global' => array());
        # loop through each column
        for ($i = 0; $i < $this->ci->input->post('iColumns', TRUE); $i++) {
            # check that field is searchable             
            if ($this->ci->input->post('bSearchable_' . $i, TRUE) == "true") {
                # get local filter
                if (strlen($this->ci->input->post('sSearch_' . $i, TRUE)) > 0) {
                    $like['local'] = array_merge($like['local'], $this->_applyColumnFilter($sColumns[$i], $this->ci->input->post('sSearch_' . $i, TRUE)));
                } 
                
                # get global filter
                if ($this->ci->input->post('sSearch', TRUE) && $this->ci->input->post('sSearch', TRUE) != "") {
                    $like['global'] = array_merge($like['global'], $this->_applyColumnFilter($sColumns[$i], $this->ci->input->post('sSearch', TRUE)));
                } 
            }
        }
    ...
    private function _createColumnsFilterStatement($like) {
        ...
        if ($str != "") {
            //$this->ci->db->or_where("(" . $str . ")"); replace with where : filter must be define  cwhere lause AND library where clause
    ...
    private function check_cType() {
        $column = $this->ci->input->post('columns', TRUE);
